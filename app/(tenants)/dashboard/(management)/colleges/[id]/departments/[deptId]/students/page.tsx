...
import { PageHeader } from "@/components/tenants/colleges/shared";
import { StudentTable } from "@/components/tenants/students/student-table";
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Card className="p-8 border shadow-none text-center">
          ...
          <Button onClick={() => router.push("/dashboard/structure/colleges")}>
            Back to Colleges
          </Button>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-background">
      ...
      <PageHeader
        title={
          ...
          `Department ${participantLabels.plural}`
        }
        subtitle={`${currentCollege?.name} · ${pagination.total} ${participantLabels.plural.toLowerCase()}`}
        ...
        actions={
          <Button
            variant="outline"
            onClick={() =>
              router.push(`/dashboard/structure/colleges/${collegeId}/students/upload`)
            }
            className="h-8 sm:h-9 text-xs sm:text-sm"
          >
            <Upload className="w-3.5 h-3.5 sm:mr-2" />
            <span className="inline">{`Upload ${participantLabels.plural}`}</span>
          </Button>
        }
      />
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-4 space-y-4">
        ...
        <Card className="p-3 border shadow-none">
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            <div className="flex-1 relative">
              ...
              <Input
                placeholder={`Search by name, email, or ${participantLabels.singular.toLowerCase()} identifier...`}
                value={searchQuery}
        ...
        {(isRefreshing || isDeleting) && (
          <ChangingLoadingState
            messages={
              isDeleting
                ? [
                    `Deleting ${participantLabels.singular.toLowerCase()}...`,
                    "Removing from records...",
                  ]
                : [
                    `Refreshing ${participantLabels.plural.toLowerCase()}...`,
                    "Syncing latest data...",
                  ]
            }
          />
        ...
        {((loading && !isRefreshing && !isDeleting) || initialLoad) &&
          students.length === 0 && (
            <ChangingLoadingState
              messages={[
                `Loading ${participantLabels.plural.toLowerCase()}...`,
                `Fetching ${participantLabels.singular.toLowerCase()} records...`,
                `Preparing ${participantLabels.singular.toLowerCase()} list...`,
              ]}
            />
          )}
            ...
            <Card className="p-8 border shadow-none">
              <div className="text-center">
                ...
                <h3 className="text-sm font-semibold text-foreground mb-1.5">
                  {`No ${participantLabels.plural.toLowerCase()} found`}
                </h3>
                <p className="text-xs text-muted-foreground mb-3">
                  {searchQuery || levelFilter !== "all"
                    ? "Try adjusting your filters"
                    : `Get started by uploading ${participantLabels.plural.toLowerCase()} to this department`}
                </p>
                {!searchQuery && levelFilter === "all" && (
                  <Button
                    onClick={() =>
                      router.push(
                        `/dashboard/structure/colleges/${collegeId}/students/upload`,
                      )
                    }
                    className="h-9"
                  >
      ...
      <DeleteConfirmationDialog
        ...
        itemIdentifier={
          studentToDelete?.matric
            ? formatParticipantIdentifier(
                { display_identifier: studentToDelete.matric } as Record<
                  string,
                  unknown
                >,
                tenant,
