...
import { EditCollegeModal } from "@/components/colleges/Modals";
...
export default function CollegeDetailPage() {
  ...
  const handleEditCollege = async (data: {
    ...
    if (!token || !currentCollege) return;

    setIsEditSubmitting(true);
    clearError();

    try {
      await updateCollege(token, currentCollege._id, data);
                  ...
                  onClick={() =>
                    router.push(
                      `/dashboard/students?college_id=${encodeURIComponent(
                        collegeId,
                      )}&ref=${encodeURIComponent(`/dashboard/colleges/${collegeId}`)}`,
                    )
                  }
          ...
          {showAddDept && (
            ...
            >
              ...
              <div className="flex gap-2">
                ...
                >
                  {loading ? (
                    <LoadingButtonContent label="Adding..." />
                  ) : (
                    "Add Department"
                  )}
                </Button>
