...
export default function ElectionAnalyticsPage() {
  ...
  const sessionStatsQuery = useAdminSessionStatsQuery(selectedSessionId);

  if (sessionsQuery.isLoading || (selectedSessionId && sessionStatsQuery.isLoading)) {
    return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Select a session to inspect turnout, candidate performance, and rejection patterns.
            </p>
          </div>
        </div>
      </section>
          ...
          {stats ? (
            <>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                {[
                  { label: "Eligible students", value: stats.stats.eligible_students },
