...
import {
  ...
  validateFullSessionCreation,
  validateSessionStep,
} from "@/components/sessions/create";
...
export function SessionBuilder({
  ...
  const normalizedFormData = useMemo(
    () => ({
      ...formData,
      eligible_levels: formData.eligible_levels.filter((level) =>
        availableLevels.includes(level),
      ),
    }),
    [availableLevels, formData],
  );

  const fullValidationIssues = useMemo(
    () => validateFullSessionCreation(normalizedFormData),
    [normalizedFormData],
  );
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const validationIssues = validateFullSessionCreation(normalizedFormData);
    if (validationIssues.length > 0) {
      ...
      const firstInvalidStep = SESSION_CREATION_STEPS.find(
        (step) => validateSessionStep(step.id, normalizedFormData).length > 0,
      );
      if (firstInvalidStep) {
        setCurrentStepIndex(
